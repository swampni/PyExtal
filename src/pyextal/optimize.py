...
class CoarseOptimize:
    ...
    def thicknessTarget(x0, *args):
        ...
        if templates.shape[0] != exitwave.shape[0] or templates.shape[1] > exitwave.shape[1] or templates.shape[2] > exitwave.shape[2]:
            raise ValueError(f'Exitwave has shape {exitwave.shape} and templates have shape {templates.shape} in thicknessTarget.')
    ...
    def displayCoarseSearch(self, filter=None, threshold=None):
        ...
        if not hasattr(self, 'loc'):
            raise ValueError('Please run optimization first to get the location of the maximum correlation.')
...
class FineOptimize:
                ...
                for j, sim in enumerate(regionSim[ith]):
                    if self.roi.maskConfig is None or self.roi.maskConfig[ith, j] == 1:  
                        ithregion.append(convolve2d(sim, 
                                                    voigt_2d(np.abs(self._probe[0]), np.abs(self._probe[1]), self.roi.padding), 
                                                    mode='valid').flatten())
                    else:
    ...
    def SFtarget(x0, *args):
        ...
        refine_param = (self.thickness,)
        if self._perturbROI:
            refine_param += (self.roi.gl, self.roi.rotation, *self.roi.allshift.flatten())
        if self.refineProbe and self._probe:
            refine_param += (self._probe[0], self._probe[1])
        roi_res = minimize(self.ROItarget, refine_param, args=(self,), method=constant['fine_geometry']['method'], options=constant['fine_geometry']['options'])
            
        if roi_res.success:
            index = 1
            self.thickness = roi_res.x[0]
            print(f'thickness: {self.thickness:.2f}', flush=True)
            if self._perturbROI:
                self.roi._gl = roi_res.x[index]
                self.roi._rotation = np.deg2rad(roi_res.x[index+1])
                self.roi.allshift = roi_res.x[index+2:index+4].reshape(self.roi.allshift.shape)
                index += 4
                # refresh the roi
                self.roi.calculatePixelSize()
                self.roi.updateExpGrid()
                print(f'gl: {self.roi.gl:.5f} rotation: {self.roi.rotation:.5f}', flush=True)
                print(f'corner: {self.roi.allshift[0,0]:.5f} {self.roi.allshift[0,1]:.5f}', flush=True)
            if self.refineProbe and self._probe:
                self._probe = (roi_res.x[index], roi_res.x[index+1])
                print(f'sigma: {self._probe[0]} gamma: {self._probe[1]}', flush=True)
    ...
    def ROItarget(x0, *args):
        ...
        index = 1
        if self._perturbROI:
            self.roi._gl = x0[index]
            self.roi._rotation = np.deg2rad(x0[index+1])
            self.roi.allshift = x0[index+2:index+4].reshape(self.roi.allshift.shape)
            index += 4
            # refresh the roi
            self.roi.calculatePixelSize()
            self.roi.updateExpGrid()
        ...
        if self._probe and self.refineProbe:
            self._probe = (x0[index], x0[index+1])
...
def voigt_2d(sigma, gamma, radius):
    ...
    """
    x = np.linspace(-radius, radius, radius*2+1)
    y = np.linspace(-radius, radius, radius*2+1)
    X, Y = np.meshgrid(x, y)
