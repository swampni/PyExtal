...
def updateSymUgh(reflections, values, sym=None):
    ...
    Note:
        the phaseDiff is a result of translation in the symmetry operation.
    """
    if sym is None:
        beams = blochwave.difpar.hkl[:,:blochwave.difpar.nbeams].T
    ...
    for ith, reflection in enumerate(reflections):
        ...
        if sym.centro:
            ...
            for i,j, phaseDiff in sym.getPos(tuple(reflection)):
                phaseDiff = np.cos(phaseDiff)
                xfre = values[ith, 0]*phase[ith, 0]*phaseDiff
                afre = values[ith, 1]*phase[ith, 1]*phaseDiff
                # print(i,j)
            ...
            for i,j, phaseDiff in sym.getPos(tuple(reflection)):
                xfre, xfim = xfmag*np.cos(xfphase + phaseDiff), xfmag*np.sin(xfphase + phaseDiff)
                afre, afim = afmag*np.cos(afphase + phaseDiff), afmag*np.sin(afphase + phaseDiff)
                # print(i,blochwave.difpar.hkl[:,:blochwave.difpar.nbeams].T[i])
                # print(j,blochwave.difpar.hkl[:,:blochwave.difpar.nbeams].T[j])
                # print('-'*20)
            ...
            # print('*'*20)
            for i,j, phaseDiff in sym.getPos(tuple(-np.array(reflection))):
                xfre, xfim = xfmag*np.cos(-xfphase + phaseDiff), xfmag*np.sin(-xfphase + phaseDiff) # conjugate
                afre, afim = afmag*np.cos(-afphase + phaseDiff), afmag*np.sin(-afphase + phaseDiff)
                # xfre, xfim = np.real(xf), np.imag(xf)
