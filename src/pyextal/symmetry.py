...
for phaseDict, it is calculated to take account for the phase difference due to translation in symmetry operation.
see Waser et al. (1955), Acta Cryst. DOI:10.1107/S0365110X55001862
For example, if we have a set of symmetry equivalent beams O, A, B
O: reference beam
A: beam that we input for structure factor adjustment
B: another symmetry equivalent beam, which we need to adjust in Ugh matrix
then the phase factor for A and B are calculated as:  
.. math::
phase_A = exp(2*pi*O*TRP_A)
$\angle F_{(A)} = phase_A + \angle F_{(O)}$
phase_B = exp(2*pi*O*TRP_B)
$\angle F_{(B)} = phase_B + \angle F_{(O)}$
given $F_{(A)}$, we can calculate $F_{(B)}$ as:
$\angle F_{(B)} = phase_B - phase_A + \angle F_{(A)}$
"""
...
class GroupSymmetry:
    ...
    def __init__(self, beams):
        ...
        self.beamIndicesGroup = [] # store this is convert by which symmetry operation
        ...
        for i in range(1, Ugh_table.shape[0]):
            for j in range(0,i,1):
                ...
                if beam not in record:                    
                    ...
                    allSym, indices = appliedSymmetry(beam)
                    self.beamGroup.append(allSym)
                    self.beamIndicesGroup.append(indices)
                    ...
                    if invBeam not in record:                                          
                        # not included in any group, add a new group
                        allSym, indices = appliedSymmetry(invBeam)
                        if self.centro:  
                            self.beamGroup[self.beam2Group[beam]].extend(allSym)
                            # this is wrong, but probably doesn't really matter
                            self.beamIndicesGroup[self.beam2Group[beam]].extend(indices)
                            for symBeam in allSym:
                                self.beam2Group[symBeam] = igroup
                                record.add(symBeam)
                        else:
                ...
                currentGroup, currentIndices = self.getGroup(beam)
                index = currentIndices[currentGroup.index(beam)]

                # calculate the phase factor for symmetry related beams, 
                # see Waser et al. (1955), Acta Cryst. DOI:10.1107/S0365110X55001862
                # if the space group contains glide/screw, the translation part will have a phase factor
                phase = np.angle(np.exp(2*np.pi*1j*np.dot(self.gp[index, 9:], currentGroup[0])))
                self.phaseDict[beam] = phase
                self.Ugh_dict[self.beam2Group[beam]].append((i,j, phase))
               
                if self.centro:
                    
                    index = currentIndices[currentGroup.index(invBeam)]
                    # need to take care of the currentGroup[0] for invBeam case
                    phase = np.angle(np.exp(2*np.pi*1j*np.dot(self.gp[index, 9:], currentGroup[0])))
                    self.phaseDict[invBeam] = phase
                    self.Ugh_dict[self.beam2Group[beam]].append((j, i, phase))
                    
                        # print(beam, invBeam, 'not found in the same group, check centro symmetry')
                else:
                    igroup = self.beam2Group[invBeam]
                    index = self.beamIndicesGroup[igroup][self.beamGroup[igroup].index(invBeam)]
                    phase = np.angle(np.exp(2*np.pi*1j*np.dot(self.gp[index, 9:], self.beamGroup[igroup][0])))
                    self.phaseDict[invBeam] = phase
                    self.Ugh_dict[igroup].append((j,i,phase))
    ...
    def getGroup(self, beam):
        ...
        """
        return self.beamGroup[self.beam2Group[beam]], self.beamIndicesGroup[self.beam2Group[beam]]
...
def appliedSymmetry(gg, sf=None):
    ...
    if sf is None: return ggs, list(unique_indices)
