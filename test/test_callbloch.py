...
def test_updateUgh():
    param = callBloch.bloch_parse(r'test/test_data/parseTest.dat')
    callBloch.updateUgh(reflections=[(1,-1,-1)], values=np.array([[10,10]]))
...
###TODO: test updateUgh noncentro

def test_updateSymUgh():
    param = callBloch.bloch_parse(r'test/test_data/parseTest.dat')
    beams = bw.difpar.hkl[:,:bw.difpar.nbeams].T

    
    three_ones_indices = []
    diff_table = np.array([[-beams[i] + beams[j] for j in range(len(beams))] for i in range(len(beams))])
    for i in range(diff_table.shape[0]):
        for j in range(diff_table.shape[1]):
            if (np.abs(diff_table[i, j, 0])==1) and (np.abs(diff_table[i, j, 1])==1) and (np.abs(diff_table[i, j, 2])==1):
                three_ones_indices.append((i, j))

    callBloch.updateSymUgh(reflections=[(1,-1,-1)], values=np.array([[10,5]]))
    for i, j in three_ones_indices:        
        assert bw.bloch_module.ughr[i, j] == 10
        assert bw.bloch_module.ughi[i, j] == -5


def test_updateSymUgh_noncentro():
    ...
    callBloch.updateSymUgh(reflections=[(1,1,1)], values=np.array([[10, 45, 0, 0]]))
    for i, j in plus:        
        print(diff_table[i, j])
        assert bw.bloch_module.ughr[i, j] == approx(10/np.sqrt(2), abs=1e-7)
        assert bw.bloch_module.ughi[i, j] == approx(10/np.sqrt(2), abs=1e-7)
    for i, j in minus:
        assert bw.bloch_module.ughr[i, j] == approx(10/np.sqrt(2), abs=1e-7)
        assert bw.bloch_module.ughi[i, j] == approx(-10/np.sqrt(2), abs=1e-7)
